...
class DebugWindow(gtk.Dialog):
    ...
    def __init__(self, parent=None):
        ...
        self.quit = self._create_button(gtk.STOCK_OK, _('_Continue'))
        self.quit.connect('clicked', self._on_send__clicked)
        self.add_action_widget(self.quit, gtk.RESPONSE_ACCEPT)
        #self.action_area.pack_end(self.quit)
...
def show(exctype, value, tb):
    if exctype is not KeyboardInterrupt:
        ...
        try:
            ...
            s,e = dw._buffer.get_bounds()
            traza =dw._buffer.get_text(s,e)
        except:
            print "error", sql, sys.exc_info[1]
        dw.run()
        dw.destroy()
...
#    
#    b = gtk.Button("Error")
#    b.connect('clicked', error_click)
#    a.connect("delete-event", gtk.mainquit)
#    a.add(b)
#    a.show_all()
#    gtk.main()
